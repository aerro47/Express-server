...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then(userFavs => {
        if (userFavs) {
            ...
            newFavs.forEach(fav => { 
                if (!userFavs.campsites.includes(fav)) { userFavs.campsites.push(fav) } });
            userFavs.save()
            .then(favs => {
                res.statusCode = 200;
                res.setHeader('Content-Type', 'applications/json');
...
.all(cors.corsWithOptions, authenticate.verifyUser, (req, res) => {
    res.statusCode = 403;
    res.setHeader('Content-Type', 'text/plain');
    ...
    .then(userFavs => {
        if (userFavs) {
            if (userFavs.campsites.indexOf(req.params.campsiteId) < 0) {
                ...
                res.statusCode = 200;
                res.setHeader('Content-Type', 'text/plain');
                res.end("That campsite is already exists in favorites!")
            }
        } else {
...
.delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then(userFavs => {
        ...
        if (delIndex > -1) {
            ...
            res.statusCode = 200;
            res.setHeader('Content-Type', 'text/plain');
            res.end('This campsite is not on the favorites list to delete!');
        }
    })
    .catch(err => next(err));
})
